...
待优化：
    ...
    3. 多线程无法加速，应该采用多进程，但需将Carla.Transform序列化
...
def main():
    try:
        ...
        bic_bp = blueprint_library.find('vehicle.gazelle.omafiets')
        ...
        for this_frame in range(total_frame):
            ...
            for this_car in all_veh_ids:
                ...
                if this_car in actor_spawn_time and int(actor_spawn_time[this_car]*10) == this_frame:
                    # print(f'this_car的type为：{type(this_car)}') # int
                ...
                # print(f'目前有的car_actor：{chapter_car_actor_list}')
                for this_car_actor in chapter_car_actor_list:
                    if this_car_actor.attributes['role_name'] == str(this_car):
                        ...
                        if int(actor_destroy_time[this_car] * 10) == this_frame:
                            ...
                            chapter_car_actor_list.remove(this_car_actor)
                ...
                for this_car_actor in chapter_car_actor_list:
                    if this_car_actor.attributes['role_name'] == str(this_car):
                        ########################################################################################
                        this_car_actor_id = int(this_car_actor.attributes['role_name'])
                        ...
                        this_car_transform = this_car_actor.get_transform()
            ...
            所有 bic
            '''
            for this_bic in all_bic_ids:
                '''
                这个 bic 要生成
                '''
                if this_bic in actor_spawn_time and int(actor_spawn_time[this_bic]*10) == this_frame:
                    # print(f'this_car的type为：{type(this_car)}') # int
                    bic_bp.set_attribute('role_name',str(this_bic)) # 将这个actor与id相挂钩，方便之后直接操控相应id的actor
                    this_bic_actor = world.spawn_actor(bic_bp, wps_dic[this_bic][0].transform)
                    chapter_bic_actor_list.append(this_bic_actor)
                    name = this_bic_actor.attributes['role_name']
                    print(f'生成 bic {name}')           # 这里测试了 role_name 成功设置上了
                    PID=VehiclePIDController(this_bic_actor,args_lateral=args_lateral_dict,args_longitudinal=args_long_dict)
                    pid_dict[this_bic] = PID
                    # # next = wps_dic[every_car][1] # 这一行是否有用
                ...
                '''
                # all_actor_this_moment = chapter_actor_list 
                # print(f'目前有的bic_actor：{chapter_bic_actor_list}')
                for this_bic_actor in chapter_bic_actor_list:
                    if this_bic_actor.attributes['role_name'] == str(this_bic):
                        # name = this_car_this_moment.attributes['role_name']
                        # print(f'现在有的actor：{name}')
                        if int(actor_destroy_time[this_bic] * 10) == this_frame:
                            this_bic_actor.destroy()
                            chapter_bic_actor_list.remove(this_bic_actor)
                            print(f'销毁 bic {this_bic}')
                ...
                for this_bic_actor in chapter_bic_actor_list:
                    if this_bic_actor.attributes['role_name'] == str(this_bic):
                        ########################################################################################
                        this_bic_actor_id = float(this_bic_actor.attributes['role_name'])
                        # print_role_name = this_actor_this_moment.attributes['role_name']
                        # print(f'车辆attribute为： {print_role_name}')
                        PID = pid_dict[this_bic_actor_id]

                        next_waypoint = wps_dic[this_bic_actor_id][this_frame - 0]
                        this_bic_transform = this_bic_actor.get_transform()
                        dist_to_next_waypoint = distance_vehicle(next_waypoint, this_bic_transform)

                        control = PID.run_step(bic_target_speed, next_waypoint)
                        
                        if dist_to_next_waypoint > 0.5: 
                            this_bic_actor.apply_control(control)
                        ########################################################################################   
        ...
        # for every_actor in chapter_car_actor_list:
        #     every_actor.destroy()

...
from PythonAPI.carla.agents.tools.misc import draw_waypoints, is_within_distance, get_speed, distance_vehicle, vector
...
from agents.tools import Veh_distance

sys.path.append(os.path.join(os.path.dirname(__file__),'ZLGCAN'))
from PythonAPI.CarSimulator.ZLGCAN.CAN_Config import CAN_Config
...
try:
    while True:
        ...
        if ego_dist < 1.5: 
            ...
            next = wps[i]
            control = PID.run_step(target_speed, next)
        distanceDiff = Veh_distance(target.get_transform(), ego.get_transform())
        if is_within_distance(target.get_transform(), ego.get_transform(), 25, [-50, 50]):
            control.throttle = 0.0
            control.brake = 0.5
            control.hand_brake = False
        current_speed = get_speed(ego)
        CAN.can.sendbuf[0].ID = 0x100
        CAN.can.sendbuf[0].DataLen = 8
        CAN.can.sendbuf[0].Data[0] = int(control.brake*200)
        CAN.can.sendbuf[0].Data[1] = int(current_speed*10)
        CAN.can.sendbuf[0].Data[2] = int(current_speed*10/256)
        CAN.can.sendbuf[0].Data[3] = int(control.throttle*200)
        CAN.can.sendbuf[0].Data[4] = int(distanceDiff*100)
        CAN.can.sendbuf[0].Data[5] = int(distanceDiff*100/256)
        CAN.can.sendbuf[0].Data[6] = 0x00
        CAN.can.sendbuf[0].Data[7] = 0x00
        CAN.can.transmit(1)

        res = CAN.can.receive()
        for i in range(res):
            if CAN.can.ID.receivebuf[i].ID == 0x123:
                control.throttle=float(CAN.can.sendbuf[i].Data[0])*0.005
                control.brake=float(CAN.can.sendbuf[i].Data[1])*0.005
                target_speed=float(CAN.can.sendbuf[i].Data[2]+CAN.can.sendbuf[i].Data[3]*256)*0.1
                ego.apply_control(control)

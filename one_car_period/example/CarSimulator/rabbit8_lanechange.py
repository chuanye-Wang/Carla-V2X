# step8: update route when front stationary vehicle 
# 8.1 determine
# 8.2 feasibility check
# 8.3 lane change path
import os
import sys
...
try:
    while True:
        ...
        if distance_vehicle(world.get_map().get_waypoint(spawn_points[48].location), ego_transform) < 0.5:
            origin = carla.Location(spawn_points[48].location)
            current_w = world.get_map().get_waypoint(spawn_points[48].location)

...
try:
    while True:
        ...
        if ego_dist < 1.5: 
            ...
            next = wps[i]
            control = PID.run_step(target_speed, next)
        distanceDiff = distance_vehicle(target.get_transform(), ego.get_transform())
        if is_within_distance(target.get_transform(), ego.get_transform(), 25, [-50, 50]):
            control.throttle = 0.0
            control.brake = 0.5
            control.hand_brake = False
        Target_throttle=control.throttle
        Target_brake = control.brake
        control.throttle =CAN_throttle
        control.brake = CAN_brake
        ego.apply_control(control)
